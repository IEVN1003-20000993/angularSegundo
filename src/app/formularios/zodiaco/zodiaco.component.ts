...
export default class ZodiacoComponent implements OnInit {
  ...
  private signosChinosArray: string[] = [
    "Caballo", "Cabra", "Mono","Gallo", "Perro", "Cerdo","Rata", "Buey", "Tigre", "Conejo", "Dragón",
    "Serpiente", 
  ];
  ...
  constructor() {
    this.formulario = new FormGroup({
      ...
      sexo: new FormControl('', Validators.required) // Solo el campo de sexo
    });
  }
  ...
  calcularSigno(): void {
    if (this.formulario.valid) {
        ...
        this.resultado = `Hola <strong>${nombre} ${aPaterno} ${aMaterno}.</strong>
                          <br><strong>Tu signo chino es  ${signo} </strong>
                          <br>Edad: <strong>${edad}</strong>
  ...
  private calcularEdad(dia: number, mes: number, anio: number): number {
    const fechaNacimiento = new Date(anio, mes - 1, dia);
    const hoy = new Date();
    let edad = hoy.getFullYear() - fechaNacimiento.getFullYear();
    const diferenciaMes = hoy.getMonth() - fechaNacimiento.getMonth();
    
    // Si aún no ha llegado el cumpleaños este año, restar un año
  ...
  private obtenerSignoChino(anio: number): string {
    const signosChinos = [
        ...
        "Perro", "Cerdo", "Rata", "Buey", "Tigre", "Conejo"
    ];
    const indice = (anio - 2000 + 12) % 12; // Ajusta para que empiece en 2000
